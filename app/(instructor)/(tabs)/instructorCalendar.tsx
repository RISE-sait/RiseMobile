...
const InstructorCalendar = () => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      // Add general events for the instructor
      ...
      // Add matches that the instructor's team is involved in
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#0C0B0B] pt-2">
      ...
      <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
        <PageTitle title="Instructor Calendar" />
        ...
        <EventListContainer date={dayjs(selectedDate).format("DD MMM YYYY")}>
          {loading ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={<Text className="text-center text-gray-300">No events scheduled.</Text>}
            />
          )}
        </EventListContainer>
      </Animated.View>
