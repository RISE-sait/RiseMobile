...
const EventListItem: React.FC<EventListItemProps> = ({ id, title, time, type = "event" }) => {
  ...
  const handlePress = () => {
    router.push(
      type === "match"
        ? `/(athlete)/screens/match-details/${id}`
        : `/(athlete)/screens/event-details/${id}`
    );
  };
  ...
  return (
    <TouchableOpacity
      ...
      className={`rounded-xl p-5 mb-4 flex-row items-center shadow-lg shadow-black ${
        type === "match" ? "bg-gold-100/10" : "bg-white-100/10"
      }`}
    >
      <View className={`p-3 rounded-lg mr-4 ${type === "match" ? "bg-gold-100/40" : "bg-gold-100/20"}`}>
        <FontAwesome6 name={iconName} size={24} color="#FCA311" />
      </View>
      ...
      <FontAwesome6 name="chevron-right" size={16} color="#FCA311" />
    </TouchableOpacity>
  );
};
